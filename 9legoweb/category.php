...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
		...
		$price_grade = $cus_range[$cat['grade']];
        foreach ($price_grade as $key => &$val) {
        	if (!empty($val)) {
        		$val['formated_start'] = $val['start'];
        		$val['formated_end'] = $val['end'];
	        	$val['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$val['start'], 'price_max'=>$val['end'], 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
	            /* 判断价格区间是否被选中 */
	            if (isset($_REQUEST['price_min']) && $val['start'] == $price_min && $val['end'] == $price_max)
	            {
	                $val['selected'] = 1;
	            }
	            else
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            if($temp_name = $db->getOne($sql))
            {
	        	 ...
	        	 * 按照原始输入的筛选条件顺序来显示
	        	 */
                if (!empty($tmp_sort_arr)) {
                	$tmp_sort_use_arr = array();
					foreach ($tmp_sort_arr as $k => $v) {
						$this_eq_tmp = trim(htmlspecialchars($v));
						foreach ($attr_list as $kt => $vt) {
							if ($vt['attr_value'] == $this_eq_tmp) {
								$tmp_sort_use_arr[$k] = $vt;
							}
						}
					}
                }
